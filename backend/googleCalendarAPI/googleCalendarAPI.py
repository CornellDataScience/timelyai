...
class GoogleCalendar:
    ...
    def create_event(
        ...
        transparency=DEFAULT_EVENT_TRANSPARENCY,
        recurrence=None,
        all_day=False,
        additional_days=0,
    ...
    def update_event(self, event_id, updates, calendar_id=DEFAULT_CALENDAR_ID):
        """
    ...
    def delete_event(self, event_id, calendar_id=DEFAULT_CALENDAR_ID):
        """
    ...
    def create_calendar(self, calendar_name):
        ...
        """
        calendar = {"summary": calendar_name, "timeZone": self.timezone}
    ...
    def find_free_slots(
        self, calendar_ids, search_date, start_hour=9, end_hour=17, duration_minutes=30
    ):
        """
        ...
        time_min = datetime.combine(search_date.date(), time(start_hour, 0)).replace(
            tzinfo=local_tz
        )
        time_max = datetime.combine(search_date.date(), time(end_hour, 0)).replace(
            tzinfo=local_tz
        )
        ...
        for calendar_id, calendar_info in free_busy_request["calendars"].items():
            print(
                f"Found {len(calendar_info.get('busy', []))} busy periods in calendar {calendar_id}"
            )
            for busy_period in calendar_info.get("busy", []):
                start = datetime.fromisoformat(busy_period["start"]).astimezone(
                    local_tz
                )
        ...
        for busy_start, busy_end in merged_busy_periods:
            ...
            )  # Ensure current_time moves forward
        ...
        for start, end in free_slots:
            ...
            print(
                f" {start.strftime('%H:%M')} - {end.strftime('%H:%M')} ({max_duration_minutes:.0f} minutes available)"
            )
        ...
        for event in events:
            ...
            if "dateTime" in event["start"]:
                start = datetime.datetime.fromisoformat(
                    ...
                    event["end"]["dateTime"].astimezone(self.tz).replace("Z", "+00:00")
                )
                duration = end - start
                ...
                category = (
                    event["summary"].split()[0] if event["summary"] else "Untitled"
                )
    ...
    def calendar_to_dataframe(
        self, calendar_ids, time_min=None, time_max=None, max_results=100
    ):
        """
        ...
        for event in all_events:
            ...
            attendees_count = (
                len(event.get("attendees", [])) if "attendees" in event else 0
            )
